...
export async function processReturn(
  saleId: string,
  returnItems: { batchId: string; quantity: number }[],
  reason: string,
) {
    ...
    await prisma.$transaction(async (tx: any) => {
      ...
      for (const rItem of returnItems) {
        const saleItem = sale.items.find(
          (i: any) => i.batchId === rItem.batchId,
        );
        if (!saleItem)
          throw new Error(`Batch ${rItem.batchId} not found in this sale`);

        if (rItem.quantity > saleItem.quantity)
          throw new Error("Cannot return more than sold");
        ...
        data: {
          ...
          items: {
            create: returnItems.map(
              (i: { batchId: string; quantity: number }) => ({
                batchId: i.batchId,
                quantity: i.quantity,
              }),
            ),
